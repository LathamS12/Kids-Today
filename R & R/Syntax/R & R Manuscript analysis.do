...
********************************************************/
	...
	pause on
	//use "C:\Users\sal3ff\Scott\Kids Today\Generated Datasets\Cross-Cohort All Controls (2 imputations)", clear //For faster estimation
	use "C:\Users\sal3ff\Scott\Kids Today\Generated Datasets\Cross-Cohort All Controls", clear //20 imputed datasets
	...
	/////////////
		...
		program OLSmod
			...
			estout * using "C:\Users\sal3ff\Scott\Kids Today\Tables/`filename' - OLS estimates", ///
		...
		OLSmod "${bounded}" 	samp		"Appendix F"
	...
	//////////////////////////////
		...
		program race_ints
			...
			estout * using "C:\Users\sal3ff\Scott\Kids Today\Tables/`filename' - Race ints (unadjusted)", ///
				starlevels(+ .1 * .05 ** .01 *** .001) ///
				cells(b(star fmt(2))  se(par(`"="("' `")""') fmt(2))) stats(r2 N) replace  ///
		...
		*race_ints "${bad_behav1}"	beh_samp	"Table 3 - Panel A"
		...
		program ses1_int
			...
			estout * using "C:\Users\sal3ff\Scott\Kids Today\Tables/`filename' - SES1 int (unadjusted)", ///
				starlevels(+ .1 * .05 ** .01 *** .001) ///
				cells(b(star fmt(2))  se(par(`"="("' `")""') fmt(2))) stats(r2 N) replace  ///
		...
		*ses1_int "${behavior1}"	beh_samp	"Table 3 - Panel B"
	...
	//Appendix A - Correlations between teacher-reported measures and direct assessments
		...
		*2010
			...
			corr X1RSCAL READ1 READ_HI2 READ_LO2  if samp ==1
	...
	************
		...
		*************************
			...
			graph export "Appendix C - Panel A.png", replace

			graph combine T1SORTS T1ORDER T1GRAPH T1MEASU T1STRAT, altshrink	
			graph export "Appendix C - Panel B.png", replace
 
 
	//Figures that show age adjusted differences across low-hi proficiency variables
			 ...
			 postfile `name' str10(group) LO_0 LO_1 HI_0 HI_1 using `file'
				...
				//Create a figure from the new dataset
					preserve
						...
						graph export "C:\Users\sal3ff\Scott\Kids Today\Figures/`title'.png", replace
